...
export default function DOthers() {
  const [activeTab, setActiveTab] = useState("All Other");
  return (
    <div className="">
      {/* Tabs */}
      <div className="w-full overflow-x-auto overflow-y-hidden [scrollbar-width:thin] [scrollbar-color:#2f4553_transparent]  [-webkit-overflow-scrolling:touch]  [transform:translate(0)] ![-moz-transform:none]">
        <div className="inline-flex bg-[#1A2C38] p-[6px] gap-2 rounded-[30px]">
          {tabs.map((tab) => (
            <button
              key={tab}
              ref={(el) => {
              ...
              className={`text-white whitespace-nowrap cursor-pointer font-semibold px-5 py-[10px] rounded-full transition-all ${
                activeTab === tab ? "bg-[#304553]" : "hover:bg-[#304553]"
              }`}>
              {tab}
            </button>
          ))}
      ...
      {/* Table */}
      <div className="bg-[#1A2C38] rounded-lg border border-gray-700 pt-[32px] mt-6">
        <div className="px-6 overflow-hidden">
          <div className="w-full overflow-x-auto scrollbar-hide">
            <table className="w-full">
              <thead className="!font-semibold">
                <tr>
                  <th className="px-4 text-left whitespace-nowrap !font-semibold !text-[#B1BAD3]">
                    Date
                  </th>
                  <th className="px-4 text-left whitespace-nowrap !font-semibold !text-[#B1BAD3]">
                    Type
                  </th>
                  <th className="px-4 text-right whitespace-nowrap !font-semibold !text-[#B1BAD3]">
                    Amount
                  </th>
                </tr>
              </thead>
        ...
        <div className="w-[100%] mt-5 p-[16px] !pb-[0px] flex justify-center">
          <div className="flex flex-col items-center justify-center h-40 p-[16px] max-w-fit text-[#B1BAD3]">
            <svg
              width="80"
              height="80"
              viewBox="0 0 80 80"
            ...
            <p className="mt-6 text-[16px]">No entries.</p>
          </div>
        </div>
      </div>
    </div>
